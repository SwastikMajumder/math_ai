...
def apply_formula(equation, formula):
    ...
    if formula.function_type == TYPE_VARIABLE:
        ...
        if formula.variable_name == "k" and is_contain_m(equation)==True:
            return False
        if formula.variable_name == "p" and equation.function_type != TYPE_CONSTANT:
            return False
...
def possible_equation_integration_substitute(equation, is_inside_int):
    ...
    if equation.function_type == TYPE_VARIABLE or equation.function_type == TYPE_CONSTANT:
        return None
    if is_inside_int and is_contain_m(equation) and is_contain_dif(equation)==0:
...
def generate_equation(equation, formula_input_list, formula_output_list):
    ...
    for i in range(len(formula_input_list)):
        ...
        if curr is not None and compare_equation(curr, equation)==False and is_contain_inf(curr)==False:
            #print_equation(curr)
        ...
        while True:
            ...
            if result[0] == False:
                ...
                curr = result[1]
            if compare_equation(curr, equation)==False and is_contain_inf(curr)==False:
                #print_equation(curr)
                generate_equation__equation_list.append(curr)
            j += 1
    ...
    if equation.function_type == ADD:
        ...
        if equation.children[0].function_type == INF:
            index = 0
        elif equation.children[1].function_type == INF:
            index = 1
        else:
            return None
        if is_contain_dif(equation.children[1-index])==False:
            tmp = duplicate_equation(equation)
            tmp_1 = duplicate_equation(equation.children[1-index].children[0].children[0])
            tmp_2 = duplicate_equation(equation.children[1-index].children[1].children[0])
            tmp_3 = equation_node(DIF, [])
            tmp_4 = equation_node(DIF, [])
            tmp_3.children.append(tmp_1)
            tmp_4.children.append(tmp_2)
            tmp.children[1-index].children[0].children[0] = tmp_3
            tmp.children[1-index].children[1].children[0] = tmp_4
            generate_equation__equation_list.append(tmp)
        tmp = duplicate_equation(equation.children[index])
        tmp = replace_instances(tmp, equation.children[1-index].children[0].children[0], equation.children[1-index].children[1].children[0])
        tmp_2 = duplicate_equation(equation)
        tmp_2.children[index]= tmp
        if compare_equation(tmp_2, equation)==False:
            #print_equation(tmp_2)
            generate_equation__equation_list.append(tmp_2)
    if len(equation.children) == 1:
        
        possible_equation_integration_substitute__attempt_list = []
        possible_equation_integration_substitute(equation, False)
        for i in range(len(possible_equation_integration_substitute__attempt_list)):
            ...
            edit_2 = equation_node(TYPE_VARIABLE, [], None, "n")
...
def add_swap(extra_eq, equation):
    ...
    while True:
        ...
        if compare_equation(curr, extra_eq)==False and compare_equation(curr, equation)==False  and find_in_equation_list(curr)==False:
            #print_equation(curr)
            add_swap__equation_list.append(curr)
        j += 1
...
def remove_duplicate():
    ...
    while i<new_size:
        j=i+1
        while j<new_size:
            if compare_equation_commutative(generate_equation__equation_list[i], generate_equation__equation_list[j]):
                k=j
                while k<new_size-1:
...
def formula_function():
    fl = "aa=a^2, 1^-1=1, abb=ab^2, a^b+a^c=a^(b+c), (a^b)*a^(-1*b)=a, dif(a^p)=p*a^(p-1)*dif(a), dif(p)=0, dif(a+b)=dif(a)+dif(b), dif(cos(a))=-sin(a)dif(a), dif(sin(a))=cos(a)dif(a), int(sin(m)dif(m))=-cos(m), a+0=a, ab+abc=ab(c+1), 0*a=0, int(k dif(m))=km, int(m*dif(m))=(m^2)(2^-1), int((a+b)dif(m))=int(a dif(m))+int(b dif(m)), int(ka dif(m))=kint(a dif(m)), a+a=2a, int(m^a dif(m))=(m^(a+1))*((a+1)^(-1)), a^1=a, a^b*a^c=a^(b+c), (ab)^c=a^c*b^c, aa^(-1)=1, 1-a^2=(1+a)(1-a), sin(a)^2=1-cos(a)^2, ab+ac=a(b+c), a+cba=a(bc+1), ab+ac=a(b+c), (a+b)(c+d)=ac+ad+bc+bd, a(b+c)=ab+ac, sin(a)^2+cos(a)^2=1, (a-b)^2=a^2-2ab+b^2, csc(a)=1/sin(a), cot(a)=cos(a)/sin(a)"
    #fl = "aa=a^2, a+a=2a"
    fl = fl.replace(" ", "")
    fl = fl.replace(",", "=")
    arr = fl.split("=")
...
#eq = string_to_equation_short("inf((int((m*dif(m)))+int((m*dif(m)))))")
#eq = string_to_equation_short("inf((((m^2)*(2^-1))+int((m*dif(m)))))")
#eq = string_to_equation_short("inf((((m^2)*(2^-1))+((m^2)*(2^-1))))")
#print(is_contain_int(eq))
#generate_equation(eq, fl[0], fl[1])
#caller_add_swap(eq, fl[0], fl[1])
#for i in generate_equation__equation_list:
#    print_equation(i)
