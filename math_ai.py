...
class equation_node:
    def __init__(
        self, function_type, children, constant_value=None, variable_name=None
    ):
...
def math_parser_remove_extra_bracket(given_string):
    ...
    while i != len(given_string):
        if given_string[i] == "(" and (
            not (is_char_cap_letter(given_string[i - 1]) if i != 0 else True)
        ):
            if find_corresponding_bracket(
                given_string, i
            ) < search_for_basic_operation_symbol(given_string, i + 1):
                given_string = remove_char_at_index(
                    given_string, find_corresponding_bracket(given_string, i)
                )
    ...
    while i != len(given_string):
        if (
            ...
            and (
                find_corresponding_bracket(given_string, i)
                - find_corresponding_bracket(given_string, i + 1)
            )
            == 1
        ):
            given_string = remove_char_at_index(
...
def merge_nested_bracket(equation):
    ...
    if (
        ...
        or equation.function_type == TYPE_CONSTANT
    ):
        return equation
    i = 0
    while i < len(equation.children):
...
def math_parser_no_subtraction(given_string):
    ...
    while (i + 1) != len(given_string):
        if given_string[i] == "-" and (
            given_string[i + 1] == "("
            or is_char_cap_letter(given_string[i + 1])
            or is_char_small_letter(given_string[i + 1])
        ):
            given_string = insert_char_at_index(given_string, "*", i + 1)
...
def merge_loop(equation):
    orig_equation = None
    while True:
        orig_equation = equation_node(
            equation.function_type,
            equation.children,
            equation.constant_value,
...
def math_parser_explicit_multiply_sign(given_string):
    ...
    while (i + 1) != len(given_string):
        if (
            given_string[i] == ")"
            or is_char_small_letter(given_string[i])
            or is_char_number(given_string[i])
        ) and (
            given_string[i + 1] == "("
            or is_char_small_letter(given_string[i + 1])
            or is_char_cap_letter(given_string[i + 1])
...
def math_parser_remove_first_bracket(given_string):
    if (
        given_string[0] == "("
        and find_corresponding_bracket(given_string, 0) == len(given_string) - 1
    ):
            ...
            while i >= 0:
                if given_string[list_operators[i]] == "^":
                    if i == len(list_operators) - 1:
                        given_string = insert_char_at_index(
                            given_string, ")", math_parser_explicit_bracket_bodmas__end
                        )
                    else:
                        given_string = insert_char_at_index(
                            given_string, ")", list_operators[i + 1]
                        )
                    if i == 0:
                        given_string = insert_char_at_index(given_string, "(", start)
                    else:
                        given_string = insert_char_at_index(
                            given_string, "(", list_operators[i - 1] + 1
                        )
            ...
            if plus_occur == 1:
                ...
                while i < len(list_operators):
                    if given_string[list_operators[i]] == "*":
                        orig = i
                        while (
                            i < len(list_operators)
                            and given_string[list_operators[i]] == "*"
                        ):
                            i += 1
                        if i == len(list_operators):
                            given_string = insert_char_at_index(
                                given_string,
                                ")",
                                math_parser_explicit_bracket_bodmas__end,
                            )
                        else:
                            given_string = insert_char_at_index(
                                given_string, ")", list_operators[i]
                            )
                        if orig == 0:
                            given_string = insert_char_at_index(
                                given_string, "(", start
                            ...
                            given_string = insert_char_at_index(
                                given_string, "(", list_operators[orig - 1] + 1
                            )
                        math_parser_explicit_bracket_bodmas__end += 2
                        break
                    i += 1
...
def math_parser_explicit_bracket_bodmas_apply(given_string, start):
    global math_parser_explicit_bracket_bodmas__end
    prev_string = given_string[
        start : math_parser_explicit_bracket_bodmas__end - start + 1
    ]
    bodmas_order = "^/*"
    i = 0
    while i < len(bodmas_order):
        given_string = math_parser_explicit_bracket_bodmas(
            given_string, start, bodmas_order[i]
        )
        while (
            prev_string
            != given_string[
...
def math_parser_string_to_equation(given_string, start, end):
    ...
    while i < end:
        if (
            not (is_char_small_letter(given_string[i]))
            and not (is_char_number(given_string[i]))
            and given_string[i] != "-"
        ):
            break
        if i == (end - 1):
    ...
    if search_for_basic_operation_symbol(given_string, start) >= end:
        ...
        operation_extract = given_string[
            search_for_basic_operation_symbol(given_string, start)
        ]
    ...
    while True:
        ...
        if curr_index >= end:
            if given_string[prev_index + 1] == "(":
                child = math_parser_string_to_equation(
                    given_string, prev_index + 2, end - 1
                )
            else:
                child = math_parser_string_to_equation(
                    given_string, prev_index + 1, end
                )
            equation.children.append(child)
            return duplicate_equation(equation)
        else:
            if given_string[prev_index + 1] == "(":
                child = math_parser_string_to_equation(
                    given_string, prev_index + 2, curr_index - 1
                )
            else:
                child = math_parser_string_to_equation(
                    given_string, prev_index + 1, curr_index
                )
        equation.children.append(child)
...
def math_parser_recursive_bodmas(given_string, start, first_time):
    global math_parser_explicit_bracket_bodmas__end
        ...
        len(given_string)
        if first_time
        else find_corresponding_bracket(given_string, start - 1)
    )
...
def math_parser_capital_letter(given_string):
    for i in range(4, len(function_full_name_list)):
        given_string = given_string.replace(
            function_full_name_list[i], chr(i - 4 + ord("A"))
        )
    return given_string
...
def convert_equation_to_string(equation, output_string):
    if equation.function_type == TYPE_VARIABLE:
        ...
        i = 0
        while i < len(equation.children):
            output_string = convert_equation_to_string(
                equation.children[i], output_string
            )
...
def search_for_basic_operation_symbol(given_string, index):
    values = []
    values.append(
        999 if given_string.find("+", index) == -1 else given_string.find("+", index)
    )
    values.append(
        999 if given_string.find("*", index) == -1 else given_string.find("*", index)
    )
    ...
    if bracket_index < min_index and min_index < find_corresponding_bracket(
        given_string, bracket_index
    ):
        return search_for_basic_operation_symbol(
            given_string, find_corresponding_bracket(given_string, bracket_index) + 1
        )
    return min_index
...
def compare_equation(equation_1, equation_2):
    ...
    if len(equation_1.children) != len(equation_2.children):
        return False
    if (
        equation_1.function_type == TYPE_VARIABLE
        and equation_1.variable_name != equation_2.variable_name
    ):
        return False
    if (
        equation_1.function_type == TYPE_CONSTANT
        and equation_1.constant_value != equation_2.constant_value
    ):
        return False
...
def apply_formula(equation, formula):
    ...
    if formula.function_type == TYPE_VARIABLE:
        if is_char_calculus(formula.variable_name):
            if equation.function_type == TYPE_VARIABLE and is_char_calculus(
                equation.variable_name
            ):
                if (
                    generate_equation_summit__variable_list[
                        ord(formula.variable_name) - ord("a")
                    ]
                    == None
                ):
                    generate_equation_summit__variable_list[
                        ord(formula.variable_name) - ord("a")
                    ] = duplicate_equation(equation)
                elif (
                    generate_equation_summit__variable_list[
                        ord(formula.variable_name) - ord("a")
                    ].function_type
                    == TYPE_VARIABLE
                    and generate_equation_summit__variable_list[
                        ord(formula.variable_name) - ord("a")
                    ].variable_name
                    == equation.variable_name
                ):
    ...
    if len(equation.children) != len(formula.children):
        return False
    if (
        formula.function_type == TYPE_CONSTANT
        and formula.constant_value != equation.constant_value
    ):
        return False
    for i in range(len(equation.children)):
...
def replace_variables(equation):
    ...
    for i in range(len(equation.children)):
        ...
        if equation.children[i].function_type == TYPE_VARIABLE:
            child = generate_equation_summit__variable_list[
                ord(equation.children[i].variable_name) - ord("a")
            ]
...
def generate_equation_summit(equation, formula_input, formula_output):
    ...
    if (
        equation.function_type == TYPE_CONSTANT
        or equation.function_type == TYPE_VARIABLE
    ):
        return None
    generate_equation_summit__variable_list = []
    for i in range(26):
    ...
    if len(equation.children) == len(formula_input.children):
        if apply_formula(equation, formula_input) == True:
            ...
            if formula_output.function_type == TYPE_VARIABLE:
                new_instance = duplicate_equation(
                    generate_equation_summit__variable_list[
                        ord(formula_output.variable_name) - ord("a")
                    ]
                )
            else:
        ...
        len(equation.children) > len(formula_input.children)
        and len(formula_input.children) >= 2
    ):
...
def compute_constant(equation):
    if (
        ...
        return None
    if (
        len(equation.children) < 2
        or equation.children[0].function_type != TYPE_CONSTANT
        or equation.children[1].function_type != TYPE_CONSTANT
    ):
        return None
    ...
    for i in range(len(equation.children)):
        ...
        if equation_iterate__index == 0:
            if formula_input is None:
                ...
                curr = generate_equation_summit(
                    equation.children[i],
                    duplicate_equation(formula_input),
                    duplicate_equation(formula_output),
                )
                if curr is not None:
                    equation.children[i] = curr
            return [True, duplicate_equation(equation)]
        tmp_result = equation_iterate(
            equation.children[i],
            duplicate_equation(formula_input),
            duplicate_equation(formula_output),
        )
...
def replace_instances(equation, instance_find, instance_replace):
    ...
    if (
        ...
        return equation
    for i in range(len(equation.children)):
        equation.children[i] = replace_instances(
            equation.children[i], instance_find, instance_replace
        )
    return equation
...
def check_inf_structure(equation):
    if equation.function_type == INF:
        ...
        for i in range(1, len(equation.children)):
            if (
                equation.children[i].function_type != MULTIPLY
                or check_inf_structure(equation.children[i]) == False
            ):
                return False
        return True
    return False
...
def possible_equation_integration_substitute_2(equation):
    ...
    if equation.function_type == TYPE_CONSTANT:
        return None
    possible_equation_integration_substitute__attempt_list.append(
        duplicate_equation(equation)
    )
    if equation.function_type == TYPE_VARIABLE:
        return None
...
def possible_equation_integration_substitute(equation, is_inside_int):
    ...
    for i in range(len(equation.children)):
        possible_equation_integration_substitute(
            equation.children[i],
            True if is_inside_int else (equation.function_type == INTEGRATION),
        )
    return None
...
def apply_formula_2(equation, input_string):
    ...
    return apply_formula(
        duplicate_equation(equation), string_to_equation_short(input_string)
    )
...
def generate_equation(equation, formula_input_list, formula_output_list):
    ...
    for i in range(len(formula_input_list)):
        ...
        curr = generate_equation_summit(
            curr,
            duplicate_equation(curr_formula_input),
            duplicate_equation(curr_formula_output),
        )
        ...
        while True:
            ...
            curr = duplicate_equation(equation)
            result = equation_iterate(
                duplicate_equation(curr),
                duplicate_equation(curr_formula_input),
                duplicate_equation(curr_formula_output),
            )
            if result[0] == False:
    ...
    if len(equation.children) == 1 and equation.function_type == INF:
        some_list = substitute_equation(
            equation, chr(ord(next_letter__best) + 1), False
        )
    ...
    if len(equation.children) >= 2 and equation.function_type == ADD:
        ...
        some_list = substitute_equation(
            new_equation, chr(ord(next_letter__best) + 1), False
        )
        for i in range(len(some_list)):
            tmp = duplicate_equation(some_list[i])
    ...
    if (
        len(equation.children) == 1
        and equation.function_type == INF
        and equation.children[0].function_type == INTEGRATION
    ):
    ...
    if (
        len(equation.children) >= 2
        and equation.children[0].children[0].function_type == INTEGRATION
    ):
...
def multiply_up_and_down(equation):
    ...
    sorted(
        possible_equation_integration_substitute__attempt_list,
        key=cmp_to_key(equation_length_cmp),
    )
    ...
    for i in range(len(possible_equation_integration_substitute__attempt_list)):
        tmp = duplicate_equation(
            possible_equation_integration_substitute__attempt_list[i]
        )
        tmp_2 = equation_node(
            MULTIPLY,
            [
                duplicate_equation(tmp),
                equation_node(
                    POWER,
                    [duplicate_equation(tmp), equation_node(TYPE_CONSTANT, [], -1)],
                ),
            ],
        )
...
def replace_inf(equation):
    if equation.function_type == ADD:
        ...
        tmp = replace_instances(
            tmp,
            equation.children[1].children[0].children[0],
            equation.children[1].children[1].children[0],
        )
        tmp_2 = duplicate_equation(equation)
...
def add_swap(extra_eq, equation):
    ...
    while True:
        ...
        if result[0] == False:
            ...
            curr = result[1]
        if (
            compare_equation(curr, extra_eq) == False
            and compare_equation(curr, equation) == False
            and find_in_equation_list(curr) == False
        ):
            # print_equation(curr)
            generate_equation__equation_list.append(curr)
        j += 1
...
def remove_duplicate():
    ...
    while i < new_size:
        ...
        while j < new_size:
            if compare_equation(
                generate_equation__equation_list[i], generate_equation__equation_list[j]
            ):
                k = j
                while k < new_size - 1:
                    generate_equation__equation_list[
                        k
                    ] = generate_equation__equation_list[k + 1]
                    k += 1
                new_size -= 1
            else:
                j += 1
...
def caller_add_swap(equation, formula_input_list, formula_output_list):
    ...
    for i in range(len(add_swap__equation_list)):
        generate_equation(
            add_swap__equation_list[i], formula_input_list, formula_output_list
        )
    remove_duplicate()
...
def make_list(f_input, f_output, equation):
    ...
    while i < len(generate_equation__equation_list):
        generate_equation__equation_list[i] = merge_loop(
            generate_equation__equation_list[i]
        )
