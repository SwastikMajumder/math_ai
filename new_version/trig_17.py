...
with open("geo_p/formula_list_1.txt", 'r') as file:
  file_content_1 = file.read()
with open("geo_p/formula_list_2.txt", 'r') as file:
  file_content_2 = file.read()
with open("geo_p/formula_list_3.txt", 'r') as file:
  file_content_3 = file.read()
with open("geo_p/formula_list_4.txt", 'r') as file:
  file_content_4 = file.read()
with open("geo_p/formula_list_5.txt", 'r') as file:
  file_content_5 = file.read()
with open("geo_p/formula_list_6.txt", 'r') as file:
  file_content_6 = file.read()
with open("geo_p/formula_list_7.txt", 'r') as file:
  file_content_7 = file.read()
with open("geo_p/formula_list_8.txt", 'r') as file:
  file_content_8 = file.read()
with open("geo_p/formula_list_9.txt", 'r') as file:
  file_content_9 = file.read()
with open("geo_p/formula_list_10.txt", 'r') as file:
  file_content_10 = file.read()
with open("geo_p/formula_list_11.txt", 'r') as file:
  file_content_11 = file.read()
with open("geo_p/formula_list_12.txt", 'r') as file:
  file_content_12 = file.read()
with open("geo_p/formula_list_13.txt", 'r') as file:
...
def unflat(equation_list, operation):
  new_eq = TreeNode(operation)
  if len(equation_list)==2:
    new_eq.children = copy.deepcopy(equation_list)
  else:
    new_eq.children.append(equation_list[-1])
    new_eq.children.append(unflat(equation_list[:-1], operation))
  return new_eq
def simply_add(equation):
  cos_2_x = """f_pow
 f_cos
  v_0
 d_2"""
 ...
 d_2"""
  ...
  for item in break_equation(equation):
    item_2 = item
    item = flatten_tree(tree_form(item))
    x = None
    if item.name == "f_add" and len(item.children) > 2:
      x = flatten_tree(item)
      x = [str_form(y) for y in x.children]
      if x and cos_2_x in x and sin_2_x in x:
        x.remove(cos_2_x)
        x.remove(sin_2_x)
        x.append("d_1")
      summation = 0
      for y in x:
        if y[:2] == "d_":
          summation += int(y[2:])
      x = [y for y in x if y[:2]!="d_"]
      x.append("d_"+str(summation))
      x = [tree_form(y) for y in x]
      x = unflat(x, "f_add")
      new_equation = str_form(replace(tree_form(equation), tree_form(item_2), x))
      break
  return new_equation 
...
def make_same(equation):
  tmp = break_equation(equation)
  tmp = [item for item in tmp if tree_form(item).name in {"f_add", "f_mul"}]
  for item in itertools.combinations(tmp, 2):
    a = tree_form(item[0])
    b = tree_form(item[1])
    if len(a.children)==len(b.children) and len(b.children)==2 and a.name==b.name and a.name in {"f_add", "f_mul"} and str_form(a.children[0])==str_form(b.children[1]) and str_form(a.children[1])==str_form(b.children[0]):
      equation = str_form(replace(tree_form(equation), a, b))
  return equation

def s(equation_list):
  return [str_form(item) for item in equation_list]
def un_s(equation_list):
  return [tree_form(item) for item in equation_list]
def two_lcm(equation):
  
  eq = tree_form(equation)
  lcm_for = """f_add
 f_div
  u_0
  u_1
 f_div
  u_2
  u_3"""
  if not apply_individual_formula_on_given_equation(eq, tree_form(lcm_for), None, None, True):
    
    return equation
  
  eq_2 = flatten_tree(eq)
  a = copy.deepcopy(eq_2.children[0].children[1])
  b = copy.deepcopy(eq_2.children[1].children[1])
  ...
  common = [item for item in s(a+b) if item in s(a) and item in s(b)]
  ...
  if len(common) == 0:
    ...
    common = unflat(common, "f_mul")
  
  """
  lcm = unflat(dif_a+dif_b+common, "f_mul")
  if len(dif_a)==1:
    dif_a = dif_a[0]
  else:
    dif_a = unflat(dif_a, "f_mul")
  if len(dif_b)==1:
    dif_b = dif_b[0]
  else:
    dif_b = unflat(dif_b, "f_mul")
  #print(print_equation(str_form(dif_b)))
  return TreeNode("f_div", [TreeNode("f_add", [TreeNode("f_mul", [eq_2.children[0].children[0], dif_b]), TreeNode("f_mul", [eq_2.children[1].children[0], dif_a])]), lcm])

def search_4(equation, depth, visited, ans):
    if depth == 0: # limit the search
        return None
    for _ in range(1):
      equation = search_2(equation, 6)
      equation = str_form(simply_mul_main(tree_form(equation)))
      equation = simply_add(equation)
      equation = make_same(equation)
    ...
    if equation == ans:
      return True
    #tmp = search_2(equation, 7)
    output = apply_individual_formula_on_given_equation(tree_form(equation), None, None, True)
    if len(output) > 0:
      
      output = [output[0]]
    else:
      output = generate_transformation(equation, 1) + generate_transformation(equation, 2)  + [str_form(two_lcm(eq))] # generate equals to the asked one
    output = list(set(output))
    if equation in output:
      output.remove(equation)
    for i in range(len(output)):
        result = search_4(output[i], depth-1, visited, ans) # recursively find even more equals
        if result is True:
            return True # hoard them
    #output = list(set(output))
    return False
...
def print_equation_helper(equation_tree):
    ...
    sign = {"f_add": "+", "f_pow": "^", "f_tan": "?", "f_mul": "*", "f_div": "/", "f_sec": "?", "f_cos": "?", "f_sin": "?", "f_cosec": "?", "f_cot": "?", "f_sub": "-"} # operation symbols
...
def print_equation(eq):
    ...
    if n_var == 0:
      ...
      eq = eq.replace("v_1", "x")
      eq = eq.replace("v_2", "z")
    if eq in {"x", "y", "z"} or eq[:2]=="d_":
      return eq.replace("d_", "")
    eq = eq.replace("d_", "")
...
def simply_mul(equation):
  ...
  while True:
    is_break = True
    for item in itertools.combinations(output.children, 2):
      if str_form(item[0]) == str_form(item[1]):
        output.children.append(TreeNode("f_pow", [item[0], tree_form("d_2")]))
        output.children = [str_form(x) for x in output.children]
        output.children.remove(str_form(item[0]))
        output.children.remove(str_form(item[0]))
        output.children = [tree_form(x) for x in output.children]
        is_break = False
        break
      elif item[0].name == "f_pow" and str_form(item[0].children[0])==str_form(item[1]):
        val = "d_" +str(int(item[0].children[1].name[2:])+1)
        output.children.append(TreeNode("f_pow", [item[1], tree_form(val)]))
        output.children = [str_form(x) for x in output.children]
        output.children.remove(str_form(item[0]))
        output.children.remove(str_form(item[1]))
        output.children = [tree_form(x) for x in output.children]
        is_break = False
        break
      elif item[1].name == "f_pow" and str_form(item[1].children[0])==str_form(item[0]):
        val = "d_" +str(int(item[1].children[1].name[2:])+1)
        output.children.append(TreeNode("f_pow", [item[0], tree_form(val)]))
        output.children = [str_form(x) for x in output.children]
        output.children.remove(str_form(item[0]))
        output.children.remove(str_form(item[1]))
        output.children = [tree_form(x) for x in output.children]
        is_break = False
        break
      elif item[1].name == "f_pow" and item[0].name == "f_pow" and str_form(item[1].children[0])==str_form(item[0].children[0]):
        val = "d_" +str(int(item[0].children[1].name[2:])+int(item[1].children[1].name[2:]))
        output.children.append(TreeNode("f_pow", [item[0].children[0], tree_form(val)]))
        output.children = [str_form(x) for x in output.children]
        output.children.remove(str_form(item[0]))
        output.children.remove(str_form(item[1]))
        output.children = [tree_form(x) for x in output.children]
  ...
  if len(output.children) == 1:
    ...
    return unflat(output.children, "f_mul")
  return output
import copy
...
def simply_mul_main(equation):
  while True:
    ...
    for item in break_equation(equation):
      orig = equation
      equation = str_form(replace(tree_form(equation), tree_form(item), simply_mul(tree_form(item))))
      if orig!=equation:
        is_break = False
        break
    if is_break:
...
print(print_equation(ans))
print()
eq = search_2(eq, 6)
print(print_equation(eq))
eq = str_form(two_lcm(eq))
print(print_equation(eq))
for _ in range(1):
  eq = search_2(eq, 7)
  print(print_equation(eq))
  eq = simply_mul_main(eq)
  eq = search_2(eq, 8)
print(print_equation(eq))
#print(search_4(eq, 10, None, ans))
