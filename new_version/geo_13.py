...
#search_7(qq, 2, None)
#print(print_equation(qq))
file = return_formula_file(12)
#print(file)
qq_list = [str_form(item) for item in tree_form(qq).children]
for item in qq_list:
  print(print_equation(item))
...
for s in range(3):
  for i in range(-3, len(file[0])):
    ...
    if s<2 and i == -1:
      ...
      tmp = line_angle(qq_list)
    elif s<2 and i == -3:
      tmp = formula_apply(qq_list)
    else:
      tmp = opposite_angle(str_form(file[0][i]), str_form(file[1][i]), qq_list)
    qq_list += tmp
  qq_list = list(set(qq_list))

qq_list = [item for item in qq_list if item[:2]!="d_"]
for item in qq_list:
  print(print_equation(item))
